...
The potential size of such an asymmetry can be studied in simulation where the \D decay has a uniform distribution over the allowed phase space; in such simulated samples, it would manifest itself as an observation of different fractional yields 
...
\subsubsection{\texorpdfstring{Effect of the phase-space efficiency profile on \ci and \si}
...
The statistical uncertainties on the shape parameters that are obtained in fits to simulated decays and in the first stage fit to data need to be propagated to the uncertainty on the obtained parameters. This is done via a bootstrapping procedure, \mccorrect{in order to do so in a way that takes correlations into account, and which \emph{does not} rely on the uncertainty estimates in the preliminary fits being accurate}~\cite{ChernickMichaelR1999Bm:a}. The uncertainty is assessed by repeating these steps many times \mccorrect{and observing the spread of obtained central values for the observables of interest}:
\begin{itemize}
    \item Each of the data sets used to determine parameters of the signal, crossfeed, and lowmass shapes that are fixed in the first-stage fit to data of Section~\ref{sec:signal_and_background_components} are re-sampled \emph{with replacement} \mccorrect{(that is, all candidates a drawn with equal probability and a given candidate can be drawn multiple times)}, drawing a number of candidates equal to the original data-set size. These are from simulation for signal and lowmass shapes, and real data for the crossfeed shapes. All of the shapes are fit again, on the re-sampled data sets.
    \item The real data set is re-sampled with replacement, drawing a number of candidates equal to the original data-set size. Then, the first-stage fit of Section~\ref{sec:signal_and_background_components} is repeated with the shapes obtained as described above, obtaining values for the remaining shape parameters.
    \item Finally, the \CP fit is repeated using the shape parameters determined in the preceding steps, but \emph{without} re-sampling the dataset. \mccorrect{(If the final data set is also resampled, the spread in central values will include the regular statistical uncertainty on the observables, which completely dominates the effect under study: the impact of the statistical uncertainty on the fixed shaped parameters.)}
\end{itemize}
The uncertainty on each observable is taken to be the standard deviation of the set of central values obtained as described above.\footnote{\mccorrect{An alternative approach that avoids resampling with replacement by  splitting the simulated decays into $n$ independent subsamples, and gauging the spread in the measured observables when the analysis is repeated based on each subsample, would require much larger simulation samples than those available. It would also have take into account that the statistical uncertainty on a parameter determined from a subsample of the simulated decays will be larger than in the nominal analysis, where the same parameter is determined based on the full simulation sample.}}  The uncertainties are less than $0.1\times 10^{-2}$ for all \DK observables and less than $0.2\times 10^{-2}$ for all \Dpi observables.
...
\begin{align}
    \alpha^i_\mathrm{default-range} = \frac{\alpha^i_{\mathrm{high-}m_\B}}{\alpha^\mathrm{all-DP}_{\mathrm{high-}m_\B}}\times \alpha^\mathrm{all-DP}_\mathrm{default-range},
\end{align}
...
The systematic uncertainty due to this effect is assigned using pseudo experiments. The study is made for the \DtoKspipi mode only, which is sufficient since it completely dominates the overall sensitivity. 
\begin{enumerate}
...
The optimal central values are determined in a maximum likelihood fit. The set of all observables for which a measurement has been made is denoted $A$, and the set of underlying physics parameters is denoted $\theta$. The physics parameters of course determine the probability density function of measurement results of $A$, $f(A|\theta)$. Given a specific set of measurement results, $A_\mathrm{obs}$, a likelihood function is defined
\begin{align}
...
\begin{figure}[tp]
    ...
    %The solutions are written on the plots, where the top number is given with \texttt{PROB} uncertainties and the bottom number with \texttt{PLUGIN} uncertainties.
    }
    \label{fig:1d_CL}
\end{figure}
