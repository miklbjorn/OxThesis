...
\begin{figure}[tb]
    ...
    \caption{Overview of the \lhcb detector reproduced from Ref.~\cite{LHCb-detector}. The individual sub detectors are described in detail in the chapter.}
...
A number of sub-dominant backgrounds are investigated using the fast-simulation package \texttt{RapidSim}~\cite{Cowan:2016tnm}. This package can decay heavy $b$ and $c$ hadrons with kinematic distributions similar to those in \lhcb $pp$ collisions, or with user defined input distributions. The decays are typically evenly distributed over phase space, but can also be handled with \evtgen~\cite{Lange:2001uf} to take involved spins and resonant structure into account. Furthermore, a smearing of the obtained momenta is implemented, based on the \lhcb resolution. 
